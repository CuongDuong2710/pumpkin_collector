...
/**
 * @title PumpkinCollectorNFT
 * @dev Halloween-themed NFT collection for Pumpkin Collector game
 * Players can mint NFTs after completing games with qualifying scores
 */
contract PumpkinCollectorNFT is ERC721, ERC721URIStorage, Ownable, ReentrancyGuard {
    ...
    uint256 public constant LEGENDARY_THRESHOLD = 2000;
    ...
    // Events
    event PumpkinMinted(address indexed player, uint256 indexed tokenId, string tier, uint256 score);
    ...
    constructor(address initialOwner) 
        ERC721("Pumpkin Collector NFT", "PUMPKIN") 
    ...
    /**
     * @dev Mint a Pumpkin NFT based on game score
     * @param sessionId Unique game session identifier
     * @param score Final game score
     * @param level Final game level reached
     * @param pumpkinsCollected Total pumpkins collected in session
     */
    function mintPumpkinNFT(
        bytes32 sessionId,
        uint256 score,
        uint256 level,
        uint256 pumpkinsCollected
        ...
        emit PumpkinMinted(msg.sender, tokenId, tier, score);
        ...
        onlyOwner 
    {
        ...
        for (uint256 i = 0; i < recipients.length; i++) {
            ...
            emit PumpkinMinted(recipients[i], tokenId, tiers[i], 0);
        }
    }
    ...
    /**
     * @dev Override _burn to handle URIStorage (Removed - not needed in v5)
     */
}